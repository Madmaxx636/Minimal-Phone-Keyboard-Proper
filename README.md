...
* **Map of the keyboard**: press `Sym` and leave it for a moment, and a popup appears with two pages (tap the tabs to switch). **Keys** is a map of the keyboard as it sits on the phone: the symbol that `Alt` types is written in the top left corner of every key, like the printed symbols, and what `Sym` does with the key (arrows, brackets, cut, copy and paste) in the bottom right corner. The numbers are on the keys that type them, so there is no row of numbers. **Extras** is a map of the additional characters of every key: what holding the key gives first, what pressing it again gives next, and the accents in italics when they are turned on. Both are built from the same tables the keys type with, so they can not disagree with the keys. The popup goes away when you press a key or let go of `Sym`, and it can be turned off under `Additional characters` in the settings.
...
* A popup that appears when `Sym` is pressed and left for a moment: a map of the keyboard with what Alt and Sym do on every key, and a map of every key's additional characters.
