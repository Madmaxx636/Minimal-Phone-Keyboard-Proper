...
* **Word Suggestions, Habits and Auto-correct**: The toolbar always has something to offer, whether or not you are in the middle of a word, and it learns how you write.
    * **While typing a word** it completes it, from the words you use and then from a built-in list of common words, as many as could be reliably sourced for the language (140,000 to 162,000 words, turn that off if you mostly type in a way that does not suit a dictionary). `Dictionary language` in the settings picks whether that list, and the built-in spell checker used when none is turned on in Android's settings, is English, Spanish or French. Once a word is finished it suggests what could follow it.
...
* The built-in lists of common words (English 162,070, Spanish 161,917, French 140,112) are derived from [FrequencyWords](https://github.com/hermitdave/FrequencyWords) by Hermit Dave (2018), which was made from the OpenSubtitles 2018 corpus (distributed by [OPUS](https://opus.nlpl.eu/)), under the [Creative Commons Attribution-ShareAlike 4.0 licence](https://creativecommons.org/licenses/by-sa/4.0/). They were cut down and cleaned up, see `app/src/main/res/raw/license_common_words_en.txt` (and `_es`, `_fr`).
