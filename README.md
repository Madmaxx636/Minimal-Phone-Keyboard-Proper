...
* **Map of symbols and additional characters**: press `Sym` and leave it for a moment, and a popup appears with two pages (tap the tabs to switch). **Symbols** has every symbol that `Alt` and `Sym` can type, laid out like the symbols page of a touch keyboard, with the key that types each one written under it (`#` is `Alt O`, `(` is `Sym I`, `£` is `Sym ⇧P`), and a tap on a symbol types it. **Extras** is a map of the keyboard with the additional characters of every key: what holding the key gives first, what pressing it again gives next, and the accents in italics when they are turned on. Both are built from the same tables the keys type with, so they can not disagree with the keys. The popup goes away when you press a key or let go of `Sym`, and it can be turned off under `Additional characters` in the settings.
...
* A popup that appears when `Sym` is pressed and left for a moment: every symbol that Alt and Sym type, laid out like a touch keyboard's symbols page with the key under each one, and a map of every key's additional characters.
